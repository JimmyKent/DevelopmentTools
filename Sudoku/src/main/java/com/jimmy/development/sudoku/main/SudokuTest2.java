package com.jimmy.development.sudoku.main;
...
public class SudokuTest2 {
    ...
    private static void backTrack(int[][] input, int position, int currentGuess, boolean isRecursion) {
        ...
        if (isRecursion) {
            ...
            System.out.println("recursion position " + position + " currentGuess " + currentGuess + " guessNext " + guessNext);

            if (guessNext == 0) { //回溯
                input[y][x] = 0;
                --position;
                //int prevGuess = input[position / 9][position % 9];
                backTrack(input, position, input[position / 9][position % 9], true);
            } else {
                input[y][x] = guessNext;
                position++;
        ...
        if (value == 0) {
            ...
            System.out.println("forward position " + position + " currentGuess " + currentGuess + " guessNext " + guessNext);
